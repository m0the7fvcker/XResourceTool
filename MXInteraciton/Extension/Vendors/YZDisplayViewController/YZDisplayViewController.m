...
/** 内容滚动视图 */
@property (nonatomic, weak) YBFYZCollectionView *contentScrollView;
...
// 懒加载内容滚动视图
- (YBFYZCollectionView *)contentScrollView
{
    if (_contentScrollView == nil) {
        ...
        YBFYZCollectionView *contentScrollView = [[YBFYZCollectionView alloc] initWithFrame:CGRectZero collectionViewLayout:layout];
...
-(void)setIsNavBarMenu:(BOOL)isNavBarMenu{
    if (isNavBarMenu) {
...
-(UIViewController *)selectedViewController{
    return self.childViewControllers[self.selectIndex];
}
...
- (void)viewDidLayoutSubviews
{
    ...
    CGFloat titleY = _isfullScreen?contentY:self.offset;
    self.titleScrollView.frame = CGRectMake(0, titleY, self.isNavBarMenu? LYNavBarTitleViewWidth :contentW , titleH);

    // 设置内容滚动视图frame
    CGFloat contentScrollY = self.isNavBarMenu ? 0 :CGRectGetMaxY(self.titleScrollView.frame);
...
- (void)setUpTitleWidth
{
    ...
    if (self.isNavBarMenu) {
        titleMargin = (LYNavBarTitleViewWidth - totalWidth) / (count + 1);
    }
...
- (UICollectionViewCell *)collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if (self.tabBarController) {
        vc.view.frame = CGRectMake(0, 0, self.contentScrollView.width, self.tabBarController.view.height-self.tabBarController.tabBar.height-self.titleHeight);
    }else{
        vc.view.frame = CGRectMake(0, 0, self.contentScrollView.width, self.view.height-self.titleHeight);
    }
...
- (void)scrollViewDidScroll:(UIScrollView *)scrollView
{
    ...
    if (_selectIndex != leftIndex) {
        _selectIndex = leftIndex;
        [self changeSelectIndex];
    }
}
